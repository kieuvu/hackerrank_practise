...
function check(currentNumber, reverseNumber, divisor) {
  const calc = (Math.abs(currentNumber - reverseNumber)) / divisor;
  if (calc == parseInt(calc)) return true;
  return false;
}
...
function beautifulDays(i, j, k) {
  ...
  for (let n = i; n <= j; n++) {
    ...
    if (check(currentNumber, reverseNumber, k)) count++;
  }
  return count;
}
